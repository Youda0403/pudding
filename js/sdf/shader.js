...
vec4 faceDecor(vec3 q){
  ...
  float scar=1.0-smoothstep(0.85,1.10,length(c/vec2(0.0075,0.058)));
...
vec3 creamOrigin(){return CROWN+vec3(0.0,-0.012,0.12);}
...
// 접시 안쪽 평평한 영역을 따라 고이며, 림을 넘지 않는다.
float sdSyrupPool(vec3 p){
  if(uSyrup<0.5)return 1e5;
  float edge=smax(length(p.xz)-syrupRadius(p),plateOutline(p.xz)+0.30,0.035);
  float top=0.016+0.040*(1.0-smoothstep(-0.23,0.0,edge));
  return smax(edge,abs(p.y-(top-0.006)*0.5)-(top+0.006)*0.5,0.015);
}
