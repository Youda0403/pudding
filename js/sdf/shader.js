...
void earSpec(out vec3 a, out float r1, out vec3 b, out float r2, out float fz, out float k){
  if(uAnimal < 0.5){        // 고양이 — 두 직선에 접하게 맞춘 값
    ...
    a=vec3(0.2500,0.6200,0.000); r1=0.190; b=vec3(0.2750,1.1900,0.000); r2=0.100; fz=0.84; k=0.020;
...
float sdMouth2(vec2 m){
  ...
  return smax(length(p/vec2(0.85,1.10)) - 0.042, p.y - 0.002, 0.006);
}
...
vec4 faceDecor(vec3 q){
  ...
  vec2 c=rot(-0.38)*(q.xy-markPosition(uScar));
  float scar=sdCapsule2(c,vec2(0.0,-0.045),vec2(0.0,0.045),0.0045);
  scar=min(scar,sdCapsule2(c,vec2(-0.018,-0.025),vec2(0.018,-0.017),0.0035));
  scar=min(scar,sdCapsule2(c,vec2(-0.018,0.000),vec2(0.018,0.008),0.0035));
  scar=min(scar,sdCapsule2(c,vec2(-0.018,0.025),vec2(0.018,0.033),0.0035));
  scar=1.0-smoothstep(-0.001,0.0025,scar);
  ...
  return vec4(mole,blush,scar,freckle)*uMarks*front;
}
...
float sdCream(vec3 q){
  ...
  vec3 p = q - CROWN;
  bool lop = uAnimal > 2.5 && uAnimal < 3.5;
  if(lop){p.x /= 0.78;p.z -= 0.13;}
  ...
  p.xz /= flute;
  return sdRoundCone(p, vec3(0.0,0.072,0.0), vec3(0.0,0.275,0.0), 0.190, 0.022)/1.9 * (lop ? 0.78 : 1.0);
}
...
vec3 cherryCenter(){
  bool lop = uAnimal > 2.5 && uAnimal < 3.5;
  return CROWN + vec3(uCream > 0.5 ? (lop ? 0.0312 : 0.040) : 0.0,
    uCream > 0.5 ? CREAM_H + 0.023 : 0.061,
    uCream > 0.5 && lop ? 0.13 : 0.0);
}
...
float sdSprinkles(vec3 q, out float id){
  ...
  if(uCream>0.5){
    bool lop=uAnimal>2.5 && uAnimal<3.5;
    if(lop){q.x/=0.78;q.z-=0.13;}
e=sdCapsule3(q,vec3(0.158305,0.931858,0.105922),vec3(0.204057,0.941656,0.095209),0.011)*(lop?0.78:1.0);if(e<d){d=e;id=6.0;}
e=sdCapsule3(q,vec3(-0.147147,0.962114,0.178231),vec3(-0.101317,0.951604,0.187881),0.011)*(lop?0.78:1.0);if(e<d){d=e;id=7.0;}
e=sdCapsule3(q,vec3(-0.004722,1.042576,-0.186635),vec3(-0.047196,1.031163,-0.167409),0.011)*(lop?0.78:1.0);if(e<d){d=e;id=8.0;}
e=sdCapsule3(q,vec3(-0.005258,1.074096,0.098869),vec3(-0.024304,1.091254,0.058289),0.011)*(lop?0.78:1.0);if(e<d){d=e;id=9.0;}
e=sdCapsule3(q,vec3(0.041496,1.114501,-0.050869),vec3(0.087006,1.117487,-0.035907),0.011)*(lop?0.78:1.0);if(e<d){d=e;id=10.0;}
  }
 return d;
}
...
float sdSyrupPool(vec3 p){
  ...
  // 표면 높이는 중앙 0.056, 가장자리 0.016. 끝은 부드럽게 얇아진다.
  float top = 0.016 + 0.040*(1.0 - smoothstep(0.55,1.0,r));
...
float plateOutline(vec2 p){
  ...
  if(uPlateStyle < 1.5){
    float r=length(p);
    if(r<0.20)return r-1.04;
    float a=atan(p.y,p.x);
    return (r-1.16-0.13*cos(6.0*a))*0.78;
  }
  if(uPlateStyle < 2.5){
...
void main(){
  ...
  if(hit.y >= 0.0){
    ...
    if(hit.y < 0.5){
      ...
      base=mix(base,pow(vec3(0.82,0.40,0.43),vec3(2.2)),marks.z*0.88);
      ...
      gloss = 3.25; envAmt = 0.40; wet = 1.15;
    }
