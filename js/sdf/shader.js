...
void earSpec(out vec3 a, out float r1, out vec3 b, out float r2, out float fz, out float k){
  if(uAnimal < 0.5){        // 고양이 — 두 직선에 접하게 맞춘 값
    ...
    a=vec3(0.2850,0.8710,0.000); r1=0.133; b=vec3(0.2850,0.8720,0.000); r2=0.133; fz=0.80; k=0.035;
  }
}
...
float sdEars(vec3 q){
  ...
  float d = ((uAnimal > 3.5 && uAnimal < 5.5) || uAnimal > 6.5)
...
void earInner(out float si, out float dep, out float t0, out float t1, out float kin, out float gap){
  if(uAnimal < 0.5){        // 고양이
    ...
    si=0.92; dep=0.34; t0=0.340; t1=0.945; kin=0.014; gap=-10.000;
  } else if(uAnimal < 3.5){ // 롭이어
    si=0.90; dep=0.40; t0=0.240; t1=0.870; kin=0.026; gap=-10.000;
  } else if(uAnimal < 4.5){ // 곰
    si=0.82; dep=0.34; t0=0.000; t1=1.000; kin=0.016; gap=-10.000;
...
float plateOutline(vec2 p){
  ...
  if(uPlateStyle < 2.5){
    ...
    return (length(p)-1.105-0.185*cos(4.0*a))*0.78;
  }
...
vec3 background(vec2 uv){
  ...
  if(uBgPattern<1.5){
    ...
    vec2 h=p/0.055;
    float diamond=max(abs(h.x)*0.90-h.y-0.80,h.y-0.02)*0.70710678;
    float lobes=min(length(h-vec2(-0.44,0.22)),length(h-vec2(0.44,0.22)))-0.46;
...
void main(){
  ...
  if(hit.y >= 0.0){
    ...
    if(hit.y < 0.5){
      ...
      float chB = 1.0 - smoothstep(-0.001, 0.001, sdCherryBody(q)-min(dBE,sdCream(q)));
      float chS = 1.0 - smoothstep(-0.001, 0.001, sdCherryStem(q)-min(min(dBE,sdCream(q)),sdCherryBody(q)));
